...
function enemyEncounter(){
    if (currentEnemy === ""){
        ...
        commonEnemy.createEnemyDescription();
    ...
    const input = createUserInputDirection();
    getUserInput().addEventListener('change',makeBattleDecision(input))
}

function makeBattleDecision(input){
    input = getUserInput();
    if(input ==='attack' || input ==='fight'){
        battle();
    }
    else if(input === 'run' || input === 'flee'){
        createElement('You maybe fled. Probably need a roll check here');
    }
}
...
function createContentElement(input){
